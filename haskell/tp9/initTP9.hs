...
trace _ (Tip x) [] = [x]
trace t (Tip x) ps = x : trace t t ps
trace t (Bin t1 _) (Gauche:ps) = trace t t1 ps
trace t (Bin _ t2) (Droite:ps) = trace t t2 ps
trace _ (Bin _ _) [] = ""
...
qsort [] = []
qsort (x:xs) = qsort [y | y <- xs, cle y <= cle x] ++ [x] ++ qsort [y | y <- xs, cle y > cle x]
  where
    cle (c, n) = (n, c)
...
-- 3.3
--
-- On suppose que la taille de l'arbre de Huffman est négligeable vs la taille du texte a compresser
--

tauxDeCompression :: String -> Float
tauxDeCompression "" = 0
tauxDeCompression l = fromIntegral bitsCompresses / fromIntegral bitsOriginaux
  where
    t = build (calculFrequence l)
    bitsCompresses = length (code t l)
    bitsOriginaux = 8 * length l
