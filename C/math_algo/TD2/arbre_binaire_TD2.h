...
/**
   ...
   @requires gauche et droit ont été initialisé avec creer_arbre.
   @requires gauche != NULL => gauche != droit 
   @ensures resultat->valeur == val
...
/**
   ...
   @requires 'a' a été initialisé avec creer_arbre. 
   @ensures si a==NULL, resultat == 0
   @ensures si a->valeur == val, resultat == 1
   @ensures si a->valeur != val, resultat == (present(a->gauche, val) || present(a->droit, val)), 
